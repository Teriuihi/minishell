...
void	read_input_write(t_cmd_data *cmd_data, int old_pid[2], int cur_pid[2], t_minishell *minishell)
{
	...
	//ft_printf("OLDPID[0] == %d, OLDPID[1] == %d, CURPID[0] == %d, CURPID[1] == %d INSIDE READINPUTWRITE %s IS CMD\n",old_pid[0], old_pid[1], cur_pid[0], cur_pid[1], cmd_data->command->command);
	if (old_pid[0]) //are both of these need to be executed?
	{
		dup2(old_pid[0], STDIN_FILENO);
		close(old_pid[0]);
...
void	control_pipes(t_cmd_data *cmd_data, int *old_pid, int *cur_pid, t_minishell *minishell)
{
	...
	else if (cmd_data->input.type == REDIRECT_INPUT)
	{
								...
								const int *cur_pid, t_minishell *minishell)
{
	...
	(void)minishell;
	//dup2(old_pid[1], STDIN_FILENO);  	//for cat << a
			...
			t_bool is_built_in, t_minishell *minishell)
{
	...
	if (ft_streq(command->command, "exit"))
		exit (0);
	ft_printf("%s is command, %s is output type\n", cmd_data->command->command, cmd_data->output.file);
	...
	if (c_pid == 0) //only in this case do we pass the child_htable
	{
		...
		{
			//ft_printf("entering isbuiltin true, %s is command\n", cmd_data->command->command);
